...
<?php 
	...
	//Define query
	$query = 'select * from customer_history order by TIME DESC';
	...
	while ( ($row = oci_fetch_array($stid, OCI_BOTH) ) != false) {
							echo "<tr><td>". $row['TIME'] . "</td><td>" . $row['FNAME'] . " " . $row['LNAME'] . "</td><td>". $row['CATEGORY'] . "</td><td>" . $row['DETAILS'] . "</td><td>".$row['QUANTITY']."</td><td>".$row['USERNAME']."</td></tr>";
	}							
	echo '</table>';
?>
</div>
